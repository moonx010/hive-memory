...
import type { GetUserContext, UserContext } from "../src/tools/index.js";
...
describe("TASK-SEC-02: user_manage admin authorization", () => {
  ...
  beforeEach(async () => {
    ...
    // We'll set up a mutable context ref for testing
    let currentCtx: Readonly<UserContext> = Object.freeze({});
    (store as unknown as { _testSetCtx: (ctx: Readonly<UserContext>) => void })._testSetCtx = (c) => { currentCtx = c; };
    const getUserContext: GetUserContext = () => currentCtx;

    registerTools(mockServer as Parameters<typeof registerTools>[0], store, getUserContext);

    // Store reference to test helper
    (handlers as unknown as { _setCtx: (ctx: Readonly<UserContext>) => void })._setCtx = (c) => { currentCtx = c; };
